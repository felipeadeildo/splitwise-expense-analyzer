  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-3 border rounded shadow">
        ...
        <p className="text-sm">
          {data.isPayment ? 'Pagamento' : 'Despesa'}:{' '}
          {formatCurrency(data.amount, 'BRL')}
        </p>
        {!data.isPayment && (
          <>
            <p className="text-sm">
              Você pagou: {formatCurrency(data.paidShare, 'BRL')}
            </p>
            <p className="text-sm">
              Você devia: {formatCurrency(data.owedShare, 'BRL')}
            </p>
          </>
        )}
    ...
    <Card className="col-span-2">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <div className="text-right">
          ...
          <p className="font-medium">
            {selectedGroup?.name || 'Nenhum grupo selecionado'}
          </p>
        </div>
      </CardHeader>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={balanceHistory}>
              ...
              <YAxis
                ...
                tickLine={{ stroke: '#cbd5e1' }}
                tickFormatter={(value) =>
                  formatCurrency(value, 'BRL', { notation: 'compact' })
                }
              />
              ...
              <Area
                ...
                dot={false}
              />
              <Bar dataKey="netAmount" name="Transação" barSize={10}>
                {balanceHistory.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={entry.netAmount >= 0 ? '#10b981' : '#ef4444'}
                  />
                ))}
              </Bar>
            </ComposedChart>
